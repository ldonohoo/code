...
const canopy = function (numLeaves) {
	...
	function breeze (intensity, numLeaves) {
		...
		//function to move an indivual leaf
		function moveLeaf(leafNo, movement, direction) {	
				switch (movement) {
					case ("wiggle") :
						let skew = 0;
					...
					case ("twist") :	
						...
						if (direction === "forward") {
							rotate =+ last;	
							direction = "back";
						} else {  // default direction is back
							rotate =- last;
							direction = "forward";
						}
		...
		function moveBranch(leaves, branchNo) {
			while (leaves.length > 0) {
				leafNo = "leaf" + leaves.pop();
				let leafid = "#leaf" + leafNo;
				leaf = document.querySelector(leafid);
				let iter = branchNo * 10;
